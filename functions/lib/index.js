...
function buildDroppedSummaryFromStats(stats) {
    const droppedSummary = [];
...
async function cloneGitHubRepository(githubUrl, projectName) {
    const tempDir = (0, path_1.join)(os.tmpdir(), `clone-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
    try {
...
async function readAllFilesInDirectory(dirPath, basePath) {
    const files = [];
...
function buildGitHubFilteringStats(allFiles, filteredFiles, githubUrl) {
...
function buildZipFilteringStats(allZipEntries, extractedFiles, zipFileName) {
    const droppedFolders = new Set();
    const droppedFiles = [];
    const oversizedFiles = [];
    const filesByDirectory = {};
...
async function readFilesFromDirectory(dirPath, basePath) {
    const files = [];
    const entries = await fs.readdir(dirPath, { withFileTypes: true });
    for (const entry of entries) {
        const fullPath = (0, path_1.join)(dirPath, entry.name);
...
async function processZipFile(zipFile, projectName) {
    const tempDir = (0, path_1.join)(os.tmpdir(), `zip-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
    try {
...
async function extractZipFileWithStats(zipBuffer, extractPath) {
    return new Promise((resolve, reject) => {
        const allEntries = [];
...
async function extractZipFile(zipBuffer, extractPath) {
    return new Promise((resolve, reject) => {
        const files = [];
        yauzl.fromBuffer(zipBuffer, { lazyEntries: true }, (err, zipfile) => {
            if (err) {
...
async function processZipFromStorage(zipPath, fileName, projectName) {
